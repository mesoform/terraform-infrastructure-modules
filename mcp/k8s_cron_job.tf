resource "kubernetes_cron_job" "self" {
  ...
  metadata {
    annotations   = lookup(each.value.cron_job.metadata, "annotations", {})
    generate_name = lookup(each.value.cron_job.metadata, "name", null) == null ? lookup(each.value.config_map.metadata, "generate_name", null) : null
    name          = lookup(each.value.cron_job.metadata, "name", null)
    labels        = lookup(each.value.cron_job.metadata, "labels", {})
    namespace     = lookup(each.value.cron_job.metadata, "namespace", null)
  }
  spec {
    concurrency_policy            = lookup(each.value.cron_job.spec, "concurrency_policy", null)
    failed_jobs_history_limit     = lookup(each.value.cron_job.spec, "failed_jobs_history_limit", null)
    starting_deadline_seconds     = lookup(each.value.cron_job.spec, "starting_deadline_seconds", null)
    successful_jobs_history_limit = lookup(each.value.cron_job.spec, "successful_jobs_history_limit", null)
    schedule                      = lookup(each.value.cron_job.spec, "schedule", null)
    ...
    job_template {
      ...
      spec {
        ...
        completions                = lookup(each.value.cron_job.spec.job_template.spec, "completions", null)
        manual_selector            = lookup(each.value.cron_job.spec.job_template.spec, "manual_selector", null)
        parallelism                = lookup(each.value.cron_job.spec.job_template.spec, "parallelism", null)
        ttl_seconds_after_finished = lookup(each.value.cron_job.spec.job_template.spec, "ttl_seconds_after_finished", null)
        dynamic "selector" {
          for_each = lookup(each.value.cron_job.spec.job_template.spec, "selector", null) == null ? {} : { selector : each.value.cron_job.spec.job_template.spec.selector }
          content {
            match_labels = lookup(selector.value, "match_labels", null)
            dynamic "match_expressions" {
              for_each = lookup(selector.value, "match_expressions", null) == null ? {} : { match_expressions : selector.value.match_expressions }
              content {
        ...
        template {
          ...
          spec {
            ...
            dns_policy                      = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "dns_policy", null)
            host_ipc                        = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "host_ipc", null)
            host_network                    = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "host_network", null)
            host_pid                        = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "host_pid", null)
            hostname                        = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "hostname", null)
            node_name                       = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "node_name", null)
            node_selector                   = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "node_selector", null)
            priority_class_name             = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "priority_class_name", null)
            restart_policy                  = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "restart_policy", null)
            //security_context                = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "security_context", null)
            service_account_name             = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "service_account_name", null)
            share_process_namespace          = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "share_process_namespace", null)
            subdomain                        = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "subdomain", null)
            termination_grace_period_seconds = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "termination_grace_period_seconds", null)

            dynamic "affinity" {
              for_each = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "affinity", null) == null ? {} : { affinity : each.value.cron_job.spec.job_template.spec.template.spec.affinity }
              content {
            ...
            dynamic "dns_config" {
              for_each = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "dns_config", null) == null ? {} : { dns_config : each.value.cron_job.spec.job_template.spec.template.spec.dns_config }
              content {
            ...
            dynamic "host_aliases" {
              for_each = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "host_aliases", null) == null ? {} : { host_aliases : each.value.cron_job.spec.job_template.spec.template.spec.host_aliases }
              content {
            ...
            dynamic "image_pull_secrets" {
              for_each = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "image_pull_secrets", null) == null ? {} : { image_pull_secrets : each.value.cron_job.spec.job_template.spec.template.spec.image_pull_secrets }
              content {
                name = lookup(image_pull_secrets.value, "name", null)
              }
            }
            dynamic "init_container" {
              for_each = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "init_container", []) == [] ? [] : [for init_container in each.value.cron_job.spec.job_template.spec.template.spec.init_container : {
            ...
            dynamic "security_context" {
              for_each = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "security_context", null) == null ? {} : { security_context : each.value.cron_job.spec.job_template.spec.template.spec.security_context }
              content {

              }
            }
            dynamic "toleration" {
              for_each = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "toleration", null) == null ? {} : { toleration : each.value.cron_job.spec.job_template.spec.template.spec.toleration }
              content {
            ...
            dynamic "volume" {
              for_each = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "volume", null) == null ? [] : [for volume in lookup(each.value.cron_job.spec.job_template.spec.template.spec, "volume", null) : {
                //for_each = lookup(each.value.cron_job.spec.job_template.spec.template.spec, "volume", []) == [] ? [] : [for volume in each.value.cron_job.spec.job_template.spec.template.spec.volume : {
