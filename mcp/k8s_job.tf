resource "kubernetes_job" "self" {
  for_each = local.k8s_job

  metadata {
    annotations   = lookup(each.value.job.metadata, "annotations", {})
    generate_name = lookup(each.value.job.metadata, "name", null) == null ? lookup(each.value.config_map.metadata, "generate_name", null) : null
    name          = lookup(each.value.job.metadata, "name", null)
    labels        = lookup(each.value.job.metadata, "labels", {})
    namespace     = lookup(each.value.job.metadata, "namespace", null)
  }
  spec {
    active_deadline_seconds    = lookup(each.value.job.spec, "active_deadline_seconds", null)
    backoff_limit              = lookup(each.value.job.spec, "backoff_limit", null)
    completions                = lookup(each.value.job.spec, "completions", null)
    manual_selector            = lookup(each.value.job.spec, "manual_selector", null)
    parallelism                = lookup(each.value.job.spec, "parallelism", null)
    ttl_seconds_after_finished = lookup(each.value.job.spec, "ttl_seconds_after_finished", null)
    dynamic "selector" {
      for_each = lookup(each.value.job.spec, "selector", null) == null ? {} : { selector : each.value.job.spec.selector }
      content {
    ...
    template {
      metadata {}
      spec {
        active_deadline_seconds         = lookup(each.value.job.spec.template.spec, "active_deadline_seconds", null)
        automount_service_account_token = lookup(each.value.job.spec.template.spec, "automount_service_account_token", null)
        dns_policy                      = lookup(each.value.job.spec.template.spec, "dns_policy", null)
        host_ipc                        = lookup(each.value.job.spec.template.spec, "host_ipc", null)
        host_network                    = lookup(each.value.job.spec.template.spec, "host_network", null)
        host_pid                        = lookup(each.value.job.spec.template.spec, "host_pid", null)
        hostname                        = lookup(each.value.job.spec.template.spec, "hostname", null)
        node_name                       = lookup(each.value.job.spec.template.spec, "node_name", null)
        node_selector                   = lookup(each.value.job.spec.template.spec, "node_selector", null)
        priority_class_name             = lookup(each.value.job.spec.template.spec, "priority_class_name", null)
        restart_policy                  = lookup(each.value.job.spec.template.spec, "restart_policy", null)
        //security_context                = lookup(each.value.job.spec.template.spec, "security_context", null)
        service_account_name             = lookup(each.value.job.spec.template.spec, "service_account_name", null)
        share_process_namespace          = lookup(each.value.job.spec.template.spec, "share_process_namespace", null)
        subdomain                        = lookup(each.value.job.spec.template.spec, "subdomain", null)
        termination_grace_period_seconds = lookup(each.value.job.spec.template.spec, "termination_grace_period_seconds", null)

        dynamic "affinity" {
          for_each = lookup(each.value.job.spec.template.spec, "affinity", null) == null ? {} : { affinity : each.value.job.spec.template.spec.affinity }
          content {
        ...
        dynamic "dns_config" {
          for_each = lookup(each.value.job.spec.template.spec, "dns_config", null) == null ? {} : { dns_config : each.value.job.spec.template.spec.dns_config }
          content {
        ...
        dynamic "host_aliases" {
          for_each = lookup(each.value.job.spec.template.spec, "host_aliases", null) == null ? {} : { host_aliases : each.value.job.spec.template.spec.host_aliases }
          content {
        ...
        dynamic "image_pull_secrets" {
          for_each = lookup(each.value.job.spec.template.spec, "image_pull_secrets", null) == null ? {} : { image_pull_secrets : each.value.job.spec.template.spec.image_pull_secrets }
          content {
            name = lookup(image_pull_secrets.value, "name", null)
          }
        }
        dynamic "init_container" {
          for_each = lookup(each.value.job.spec.template.spec, "init_container", []) == [] ? [] : [for init_container in each.value.job.spec.template.spec.init_container : {
        ...
        dynamic "security_context" {
          for_each = lookup(each.value.job.spec.template.spec, "security_context", null) == null ? {} : { security_context : each.value.job.spec.template.spec.security_context }
          content {

          }
        }
        dynamic "toleration" {
          for_each = lookup(each.value.job.spec.template.spec, "toleration", null) == null ? {} : { toleration : each.value.job.spec.template.spec.toleration }
          content {
        ...
        dynamic "volume" {
          for_each = lookup(each.value.job.spec.template.spec, "volume", null) == null ? [] : [for volume in lookup(each.value.job.spec.template.spec, "volume", null) : {
            //for_each = lookup(each.value.job.spec.template.spec, "volume", []) == [] ? [] : [for volume in each.value.job.spec.template.spec.volume : {
  ...
  wait_for_completion = lookup(each.value.job, "wait_for_completion", true)
}
