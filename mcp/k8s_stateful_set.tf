...
resource "kubernetes_stateful_set" "self" {
  ...
  metadata {
    ...
    generate_name = lookup(each.value.stateful_set.metadata, "name", null) == null ? lookup(each.value.stateful_set.metadata, "generate_name", null) : null
    name          = lookup(each.value.stateful_set.metadata, "name", null)
    labels        = lookup(each.value.stateful_set.metadata, "labels", null)
    namespace     = lookup(each.value.stateful_set.metadata, "namespace", null)
  }
  //noinspection HILUnresolvedReference
  spec {
    ...
    template {
      metadata {
        annotations   = lookup(each.value.stateful_set.spec.template.metadata, "annotations", null)
      ...
      dynamic "spec" {
        ...
        content {
          ...
          host_ipc                        = lookup(spec.value, "host_ipc", null)
          host_network                    = lookup(spec.value, "host_network", null)
          host_pid                        = lookup(spec.value, "host_pid", null)
          ...
          termination_grace_period_seconds = lookup(spec.value, "termination_grace_period_seconds", null)

          dynamic "affinity" {
            for_each = lookup(spec.value, "affinity", null) == null ? {} : { affinity : spec.value.affinity }
            content {
          ...
          dynamic "dns_config" {
            for_each = lookup(spec.value, "dns_config", null) == null ? {} : { dns_config : spec.value.dns_config }
            content {
          ...
          dynamic "host_aliases" {
            for_each = lookup(spec.value, "host_aliases", null) == null ? {} : { host_aliases : spec.value.host_aliases }
            content {
          ...
          dynamic "image_pull_secrets" {
            for_each = lookup(spec.value, "image_pull_secrets", null) == null ? {} : { image_pull_secrets : spec.value.image_pull_secrets }
            content {
          ...
          dynamic "security_context" {
            for_each = lookup(spec.value, "security_context", null) == null ? {} : { security_context : spec.value.security_context }
            content {

            }
          }
          dynamic "toleration" {
            for_each = lookup(spec.value, "toleration", null) == null ? {} : { toleration : spec.value.toleration }
            content {
          ...
          dynamic "volume" {
            for_each = lookup(spec.value, "volume", null) == null ? [] : [for volume in lookup(spec.value, "volume", null) : {
              //for_each = lookup(spec.value, "volume", []) == [] ? [] : [for volume in spec.value.volume : {
    ...
    dynamic "volume_claim_template" {
      ...
      content {
        metadata {
          ...
          generate_name = lookup(volume_claim_template.value.metadata, "name", null) == null ? lookup(each.value.stateful_set.metadata, "generate_name", null) : null
          name          = lookup(volume_claim_template.value.metadata, "name", null)
          labels        = lookup(volume_claim_template.value.metadata, "labels", null)
